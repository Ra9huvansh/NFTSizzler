...
contract Encoding {
    function combineString() public pure returns(string memory){
        return string(abi.encodePacked("Hi Mom! ", "Love you!"));
    }
    ...
    // Let's compile this contract in hardhadt or remix, and you'll see the "bytecode" output - that will be sent when creating a contract.
    ...
    // there is nothing such as decodePacked() as it is not possible decode back from encodedPacked form

    function multiStringCastPacked() public pure returns(string memory) {
        string memory someString = string(multiEncodePacked());
        return someString;
    }
    // simple typecasting to string() works because encodePacked() and typecasting works different internally
    // as we already saw with encodeStringPacked() and encodeStringBytes().
    ...
    // Solidity has some more "low-level" keywords, namely "staticcall" and "call". We've used call in
    // Raffle contract but I haven't really explained what was going on.

    // call: How we call functions to change the state of the blockchaih.
